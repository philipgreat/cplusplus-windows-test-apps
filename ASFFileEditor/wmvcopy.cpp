...
// Desc: Constructor.
//------------------------------------------------------------------------------
CWMVCopy::CWMVCopy():m_SeriesLength(4096)
{
    //
    // Set the reference count to 1 when creating this class.
    //
	memset (m_Videomark, 0, sizeof(m_Videomark));
	this->m_RandomSeries=new BYTE[m_SeriesLength];
	isDescrable = FALSE;
    m_cRef = 1;
}
    ...
    for( DWORD i = 0; i < m_dwStreamCount; i++ )
    {
        ...
        if( FAILED( hr ) )
        {
            break;
				...
				// 设置Video mark 为 1
		GUID Guid;              
		pStreamConfig->GetStreamType(&Guid);
		if( Guid == WMMEDIATYPE_Video && i < sizeof(m_Videomark) ) {
			m_Videomark[ m_pwStreamNumber[i]  ] = 1;
			printf("found video stream %u \n", m_pwStreamNumber[i]);
			//edited by suddy
		}
...
#define APPEND_LEN	    (16)	// 附加数据长度
#define SERIES_LEN      (4096)
								 ...
								 /* [in] */ void __RPC_FAR * pvContext)
{
	...
	if( m_pWriterAdvanced == NULL )  return -1;
	...
	//获取长度,同时给视频加上16个字节
	if(( wStreamNum <= sizeof(m_Videomark) ? m_Videomark[wStreamNum] : 0 )
		&&pSample->GetLength(&length)==S_OK
		&&pSample->SetLength(length + APPEND_LEN)==S_OK)
	{
		//printf("%u \n",wStreamNum);
		//重新获取Buffer的大小
		if(pSample->GetBufferAndLength(&pBuffer, &length) == S_OK)
		{	
			length = length - APPEND_LEN;
			//将后面的16个字节置0
			memset(pBuffer + length,0x00,APPEND_LEN);
			//检查是否需要Hash				
		
			*(pBuffer + length) = 'I';
			*(pBuffer + length+1) = 'P';
			*(pBuffer + length+15) = 'T';
			//*(DWORD*)(pBuffer + length + 2) = m_CWArray[0].nCWID;
			//*(DWORD*)(pBuffer + length + 6) = m_CWArray[1].nCWID; 
			*(pBuffer + length + 14) = 0;
			// 异或
			for(DWORD i = 0; i<length; i ++)
			{
				*(pBuffer+i) = *(pBuffer+i) ^ this->m_RandomSeries[i % SERIES_LEN];
			}
    ...
    if( FAILED( hr ) )
    {
		printf("tracing a error %08X stream no.: %u \n",hr,wStreamNum);
        m_hr = hr;
        SetEvent( m_hEvent );
    }
...
void CWMVCopy::GenerateRandomSeries(DWORD key)
{
	DWORD dwF = key; 
	for(DWORD i=0; i<this->m_SeriesLength; i++)
	{
		
		*(this->m_RandomSeries+i) = (BYTE)dwF;//取dwF的低8bit赋值给btSeries[i]
		dwF = dwF*(0xFFFFFFFF-dwF)/16; // 迭代
		dwF = (dwF < 0xFFFFFFFF) ? dwF : 0xFFFFFFFE ;   
		dwF = (dwF > 1) ? dwF : 1 ;  
	}
}

void CWMVCopy::SetKey(DWORD key)
{
	this->GenerateRandomSeries(key);
}
