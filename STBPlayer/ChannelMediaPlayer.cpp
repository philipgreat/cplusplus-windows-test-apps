...
#define JIF(x) if (FAILED(hr=(x))) \
	...
	TCHAR msg[1024]; AMGetErrorText(hr,msg,1024);  wprintf(TEXT("\n0X%08X: %s In: "),hr,msg);\
	printf(#x); printf("\n"); return hr;}
#else
#define JIF(x) if (FAILED(hr=(x))) \
    {\
...
HRESULT CChannelMediaPlayer::RenderFile(const WCHAR *url,HWND owner)
{
    ...
    //JIF(mpVideoWindow->put_WindowStyle(WS_CHILD | WS_CLIPSIBLINGS));
	...
	//BringWindowToTop((HWND)this->mpVideoWindow);
	//SetFocus(owner);
