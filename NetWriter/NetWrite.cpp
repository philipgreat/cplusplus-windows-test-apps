...
// Desc: Constructor.
//----------------------------------------------------------------------------
/*密钥控制生成一个混沌随机序列，输出m_series*/ /*heqy*/
...
void random_series(DWORD dwKey,DWORD dwLength, BYTE btSeries[]) 
{
   DWORD dwF = dwKey; 
   if( dwLength<=0 ) 
   {
   ...
   {
	   btSeries[i] = *(BYTE *)&dwF;
       dwF = dwF*(0xFFFFFFFF-dwF)/16;
	   dwF = (dwF < 0xFFFFFFFF) ? dwF : 0xFFFFFFFE ;
	   dwF = (dwF > 1) ? dwF : 1 ;
   }
}

/*倒置置乱*/
void encrypt_disturb(BYTE pBuf[], DWORD length)
{
	BYTE pBufm;
	for(DWORD i=0; i<(int)(length/2); i++)
	{
		pBufm = pBuf[i];
		pBuf[i] = pBuf[length-i-1];
		pBuf[length-i-1]=pBufm;
	}
}
/*
void random_series1(DWORD length=0,BYTE m_series[]=NULL)
{
	if(length<=0)return;
	...
	if(mark < 5 && mark>1)
	{
		for(DWORD i=0;i<mark;i++)
			random_series1(5,m_series+i);
	}
	else if(mark = 1)
	{
		BYTE pBufm[5];
...
CNetWrite::CNetWrite()
{
    ...
    m_qwTime		= 0;
    m_hrAsync		= S_OK;
	memset( (void *)dwVideomark, 0, sizeof(dwVideomark) * sizeof(BYTE) ); /*heqy*/
	memset( (void *)btMSeries, 0, sizeof(btMSeries) * sizeof(BYTE) ); /*heqy*/
	dwCWID1         = 0; /*heqy*/
	dwCWID2         = 0; /*heqy*/
	dwCW1           = 0; /*heqy*/
	dwCW2           = 0; /*heqy*/
...
static void PrintBuffer(BYTE *pBuffer,int len)
{
	BYTE *p=pBuffer;
	printf("len = %d\n",len);
	for(int i=0; i<len; i++)
...
#define WITH_ENCRYPT 1
static  int counter=0;

static DWORD GetCWById(DWORD cwid)
{
		return cwid*cwid;
}
                                  ...
                                  void __RPC_FAR *pvContext )
{
    if( m_pWriterAdvanced != NULL )
    {
		BYTE* pBuffer; /*heqy*/
		DWORD pLength; /*heqy*/
		if( wStreamNum <= sizeof(dwVideomark) ? dwVideomark[wStreamNum-1] : 0 ) /*heqy*/
		{
#if 0
			if(pSample->GetBufferAndLength(&pBuffer,&pLength) == S_OK)
				
				if( pSample->SetLength( pLength + 16 ) == S_OK )
				{
					*(pBuffer+pLength) = 'I';
					*(pBuffer+pLength+1) = 'P';
					*(pBuffer+pLength+15) = 'T';
					//dwCWID1=135;
					if(counter%10==0){
						dwCWID1++;
						dwCWID2=dwCWID1+1;
						this->setCW( dwCWID1, dwCWID2, dwCWID1*dwCWID1, dwCWID2*dwCWID2, 1);
						//printf("%u\n",dwCWID1);
						
					}
					counter++;
					*(DWORD*)(pBuffer+pLength+2) = dwCWID1;
					*(DWORD*)(pBuffer+pLength+6) = dwCWID2; 

					//rand(
					for(DWORD i = 0; i<pLength; i ++)
					{
						*(pBuffer+i) = *(pBuffer+i) ^btMSeries[i%4096];
					}
...
HRESULT CNetWrite::Configure( DWORD dwPortNum, const WCHAR *pwszFile, UINT nMaxClient, const WCHAR *pwszServerURL )
{
	...
	do
	{
			...
			for ( DWORD i = 0; i < dwStreams; i++ )
			{
				...
				GUID myGuid;                           /*heqy*/
		        pStream->GetStreamType(&myGuid);       /*heqy*/
				if( myGuid == WMMEDIATYPE_Video && i < sizeof(dwVideomark) ) dwVideomark[i] = 1; /*heqy*/
...
/*heqy*/
void CNetWrite::setCW( DWORD CWID1, DWORD CWID2, DWORD CW1, DWORD CW2, BYTE Ver )  
{
	dwCWID1 = CWID1;
	dwCWID2 = CWID2;
	dwCW1 = CW1;
	dwCW2 = CW2;
	btVer = Ver;
	random_series( CW1, sizeof(btMSeries), btMSeries );
}
