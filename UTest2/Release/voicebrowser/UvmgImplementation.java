...
public class UvmgImplementation implements IVoiceGateway {
	...
	public int answerCall(int resourceId, int callId) {
		...
		try {
			return service.sgAnswerCall(resourceId, callId);
	...
	public int blindMakeCall(IntHolder resourceId, IntHolder callId,
		...
		// TODO Auto-generated method stub
		try {
			return service.sgBlindMakeCall(src, dest, timeout, resourceId,
					callId);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return UVMG_DOWN;
	...
	public boolean initialize(Properties properties) {
		// TODO Auto-generated method stub
		// voiceRoot="d:\\voicefiles";
		this.properties = properties;
		voiceRoot = properties.getProperty(VOICE_ROOT_CONFIG);
		if (voiceRoot == null) {
			this.fatalMessage = "语音文件不能设置为空";
			return false;
		}

		proxyLocation = properties.getProperty(PROXY_CONFIG_ITEM);
		try {
			ic = Ice.Util.initialize();
			base = ic.stringToProxy(proxyLocation);
			service = VGServicePrxHelper.checkedCast(base);
			service.VGInitialize(0, "initarg1", "initarg2");
		} catch (Exception e) {
			this.fatalMessage = "VGProxy没有启动起来或者配置错误: " + e.toString();
			return false;
		}
	...
	public int playFile(int resourceId, int callId, String filePath,
		...
		String realPath = StringTool.calcRelativePath(this.voiceRoot, filePath);
		try {
			return service.mgPlayVoice(resourceId, callId, realPath,
					interruptKeys, rate);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return UVMG_DOWN;
	...
	public int releaseCall(int resourceId, int callId) {
		// TODO Auto-generated method stub
		try {
			return service.sgReleaseCall(resourceId, callId);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return UVMG_DOWN;
		}
	...
	public int stopIO(int resourceId, int callId) {
		// TODO Auto-generated method stub
		try {
			return service.mgStopIO(resourceId, callId);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return UVMG_DOWN;
		}
	...
	public static String translateEvent(int code) {
		// TODO Auto-generated method stub
		Integer key = new Integer(code);
		return (String) eventmap.get(key);
	}

	public static String translateReason(int code) {
		// TODO Auto-generated method stub
		Integer key = new Integer(code);
		return (String) reasonmap.get(key);
	}
	...
	private VGServicePrx service;

	public UvmgImplementation() {
	...
	public int getDTMF(int resourceId, int callId, int nKeyCount,
		...
		try {
			return service.mgReceiveDTMF(resourceId, callId, nKeyCount,
					szInteruptKeys, nMax1stkeyTime, nTimeBetweenKey);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return UVMG_DOWN;
		}
	...
	public VoiceBrowserEvent getMediaEvent(int timeoutMilliSeconds) {
		// TODO Auto-generated method stub

		MGEventHolder eh = new MGEventHolder();
		VoiceBrowserEvent event = null;
		MGEvent mgevent = null;
		int state = 0;
		try {
			service.GetMgEvt(timeoutMilliSeconds, eh);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return null;
		}
		// (timeoutMilliSeconds, eh);
		// (timeoutMilliSeconds, eh);
		mgevent = eh.value;
		if (state == 0) {
			event = new VoiceBrowserEvent();
			event.setSource("mg");
			event.setEventId(mgevent.EventID);
			event.setCallId(mgevent.CallID);
			event.setResourceId(mgevent.ResID);
			event.setReasonId(mgevent.Reason);
			event.setEventData(mgevent.DTMFString);
			// event.setCallId(state);
		}
		return event;

	}

	public VoiceBrowserEvent getSignalEvent(int timeoutMilliSeconds) {
		// TODO Auto-generated method stub
		SGEventHolder eh = new SGEventHolder();
		VoiceBrowserEvent event = null;
		SGEvent sgevent = null;
		int state = 0;
		try {
			state = service.GetSgEvt(timeoutMilliSeconds, eh);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return null;
		}
		...
		if (state == 0) {
			event = new VoiceBrowserEvent();
			event.setSource("sg");
			event.setReasonId(sgevent.EventData);
			event.setEventId(sgevent.EventID);
			event.setCallId(sgevent.CallID);
			event.setResourceId(sgevent.ResID);
	...
	public int clearDtmfBuffer(int resourceId, int callId) {
		// TODO Auto-generated method stub
		try {
			return service.mgFlushBuffer(resourceId, callId);
		} catch (Ice.SocketException e) {
			this.reinitialize();
			return UVMG_DOWN;
		}
